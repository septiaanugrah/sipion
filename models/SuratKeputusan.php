 ...
 */
class SuratKeputusan extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    public function rules()
    {
        return [
            [[], 'required'],
            [['kode_surat','tanggal', 'tahun', 'created_at', 'updated_at', 'perihal'], 'safe'],
            [['created_by', 'updated_by', 'ketersediaan', 'no_urut'], 'integer'],
            [['perihal', 'penanggung_jawab'], 'string', 'max' => 500],
    ...
    protected function setNoUrut()
    {
        ...
        if(date('Y')==2020)
            $jumlahUrut = $jumlahUrut + 88;
        $this->nomor_surat = 'Kpts.' . ($jumlahUrut + 1) . '/' .$bulanRomawi . '/' .$year;
