...
use app\models\LaporanSuratRapid;
...
class LaporanSuratRapidController extends \yii\web\Controller
{
    ...
    public function actionDownload()
    {
        ...
        if($model->load(Yii::$app->request->post()))
        {
            $data = $model->getDataLaporan();
                ...
                $fileName = 'Laporan Surat Rapid SIRARA (' . $data['judulDokumen'] . ')';
                ...
                $spreadsheet->getActiveSheet()->mergeCells('A1:H1')
                    ->setCellValue('A1', 'LAPORAN SURAT KETERANGAN RAPID')
                    ->getStyle('A1')
                ...
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('D'. $baseHeader, 'Nama')
                    ->setCellValue('E'. $baseHeader, 'Tanggal Lahir')
                    ->setCellValue('F'. $baseHeader, 'NIK')
                    ->setCellValue('G'. $baseHeader, 'Alamat')
                    ->setCellValue('H'. $baseHeader, 'Hasil')
                ;
                ...
                foreach ($data['data'] as $d) {
                    ...
                    $spreadsheet->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('D' . $baseRow, $d->nama)
                        ->setCellValue('E' . $baseRow, $d->tanggal_lahir)
                        ->setCellValue('F' . $baseRow, $d->nik)
                        ->setCellValue('G' . $baseRow, $d->alamat)
                        ->setCellValue('H' . $baseRow, $d->hasil);
                    $no++;
                    $baseRow++;
                }
                ...
                $spreadsheet->getActiveSheet()->getStyle('A5:H'. ($baseRow-1))
                    ->applyFromArray($styleArray);
