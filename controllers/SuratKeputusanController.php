...
use app\models\SuratKeputusanSearch;
...
class SuratKeputusanController extends Controller
{
    ...
    {
        $searchModel = new SuratKeputusanSearch();
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->save();
                if ($model->file && $model->validate()) {                
                    $model->file->saveAs('uploads/surat-keputusan/' . Yii::$app->controller->id . '-' . $model->id . '.' . 'pdf');
                }
    ...
    { 
        ...
        // $model->file = UploadedFile::getInstance($model, 'file');
        $path = Yii::getAlias('@webroot').'/uploads/surat-keputusan/'. Yii::$app->controller->id . '-' . $id . '.' . 'pdf';
    ...
    public function actionDelete($id)
    {
        ...
        $files = FileHelper::findFiles('uploads/surat-keputusan/', [
            'only' => [Yii::$app->controller->id . '-' . $model->id . '.' . 'pdf'],
        ]);
        
        if($files)
        {
        unlink('uploads/surat-keputusan/' . Yii::$app->controller->id . '-' . $model->id . '.' . 'pdf');
        }
        $model->save(); 
    ...
    {
        if (($model = SuratKeputusan::findOne($id)) !== null) {
            return $model;
        }
